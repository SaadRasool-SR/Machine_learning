...
clust =10
bc_em, bc_em_labels, probs, score, aic, bic= em(data=scaled_cancer_dataset_x, n_components=clust,tot=1e-4)

scaler = StandardScaler()
scaled_cancer_dataset_x_gmm = scaler.fit_transform(probs)

# Split the dataset into train and test sets
X_train, X_test, y_train, y_test = train_test_split(scaled_cancer_dataset_x_gmm, breast_cancer_dataset_y, test_size=0.3, random_state=42, shuffle = True)
X_train = torch.tensor(X_train,dtype=torch.float32)
X_test = torch.tensor(X_test,dtype=torch.float32)
y_train = torch.tensor(y_train.to_numpy(),dtype=torch.float32).reshape(-1,1)
y_test = torch.tensor(y_test.to_numpy(),dtype=torch.float32).reshape(-1,1)
...
avg_accuracy_train_gmm = np.zeros((1, 100))
avg_accuracy_test_gmm = np.zeros((1, 100))
avg_f1_score_ada_gmm =np.zeros((1, 100))
avg_run_time_gmm =[]
...
class nn_model_ica(nn.Module):
    def __init__(self):
        super().__init__()
        self.input_layer = nn.Linear(in_features=10, out_features=8)
        self.act1 = nn.ReLU()
        self.hidden2 = nn.Linear(8, 64)
        self.act2 = nn.ReLU()
        self.output = nn.Linear(64, 1)
...
for i in range(number_of_iterations):
# # training 
    start_time_with_gmm = time.time()
    epoch_train_gmm = np.empty((0,))
    loss_train_gmm = np.empty((0,))
    ann_model = nn_model_ica()
    loss_function = nn.BCELoss()
    optimizer = torch.optim.Adam(ann_model.parameters(),lr=learning_rate_gmm)
    accuracy_train_gmm = np.empty((0,)) #[]
    accuracy_test_gmm = np.empty((0,)) #[]
    f1_score_ada_gmm = np.empty((0,)) #[]
    for epoch in range(n_epochs_gmm):
        for i in range(0, len(X_train), batch_size_gmm):
            Xbatch = X_train[i:i+batch_size_gmm]
            Ybatch = y_train[i:i+batch_size_gmm]
            y_pred = ann_model(Xbatch)
            loss = loss_function(y_pred, Ybatch)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        epoch_train_gmm = np.append(epoch_train_gmm,epoch)
        loss_train_gmm = np.append(loss_train_gmm,loss.data.item())

        # compute accuracy (no_grad is optional)
        with torch.no_grad():
            y_pred = ann_model(X_train)
        accuracy = (y_pred.round() == y_train).float().mean()
        accuracy_train_gmm =np.append(accuracy_train_gmm,accuracy)
        # testing
        with torch.no_grad():
            pred_test = (ann_model(X_test) > 0.5).int()
        accuracy_tes = (pred_test == y_test).float().mean()
        accuracy_test_gmm=np.append(accuracy_test_gmm,accuracy_tes)
        f1_ada = f1_score(y_test, pred_test)
        f1_score_ada_gmm=np.append(f1_score_ada_gmm,f1_ada)
    end_time_with_gmm = time.time()
    total_time_with_gmm = end_time_with_gmm - start_time_with_gmm

    avg_run_time_gmm.append(total_time_with_gmm)
    avg_accuracy_train_gmm = avg_accuracy_train_gmm + accuracy_train
    avg_accuracy_test_gmm = avg_accuracy_test_gmm + accuracy_test
    avg_f1_score_ada_gmm = avg_f1_score_ada_gmm + f1_score_ada

avg_accuracy_train_gmm = avg_accuracy_train_gmm/number_of_iterations
avg_accuracy_test_gmm = avg_accuracy_test_gmm/number_of_iterations
avg_f1_score_ada_gmm = avg_f1_score_ada_gmm/number_of_iterations
avg_run_time_gmm = sum(avg_run_time_gmm)/number_of_iterations
...
avg_accuracy_train_gmm_std = np.std(avg_accuracy_train_gmm[0],axis=0)
avg_accuracy_test_gmm_std = np.std(avg_accuracy_test_gmm[0],axis=0)
avg_f1_score_ada_gmm_std = np.std(avg_f1_score_ada_gmm[0],axis=0)

fig = go.Figure()
# Add traces
fig.add_trace(go.Scatter(x=epoch_train, y=avg_accuracy_train[0],
                    mode='lines',
                    name='Train Without gmm'))

fig.add_trace(go.Scatter(x=epoch_train, y=avg_accuracy_test[0],
                    mode='lines',
                    name='Test without gmm'))
fig.add_trace(go.Scatter(x=epoch_train, y=avg_f1_score_ada[0],
                    mode='lines',
                    name='F1 Score Without gmm'))

fig.add_trace(go.Scatter(x=epoch_train, y=avg_accuracy_train_gmm[0],
                    mode='lines',
                    name='Train With gmm'))

fig.add_trace(go.Scatter(x=epoch_train, y=avg_accuracy_test_gmm[0],
                    mode='lines',
                    name='Test With gmm'))
fig.add_trace(go.Scatter(x=epoch_train, y=avg_f1_score_ada_gmm[0],
                    mode='lines',
                    name='F1 Score With gmm'))
...
total_times= [round(avg_run_time_gmm,3), round(avg_run_time_without,3)]
experiments = ['With Gmm', 'Without Gmm']
colors = ['blue', 'green']
...
clust =10
#bc_em, bc_em_labels, probs, score, aic, bic= em(data=scaled_cancer_dataset_x, n_components=clust,tot=1e-4)
kmeans_cluster, centroids, labels = kmeans_algo(scaled_cancer_dataset_x, clust, init, max_iter, tol, algo)
scaler = StandardScaler()
scaled_cancer_dataset_x_gmm = scaler.fit_transform(breast_cancer_dataset_x)
...
class nn_model_ica(nn.Module):
    def __init__(self):
        super().__init__()
        self.input_layer = nn.Linear(in_features=31, out_features=8)
        self.act1 = nn.ReLU()
        self.hidden2 = nn.Linear(8, 64)
        self.act2 = nn.ReLU()
        self.output = nn.Linear(64, 1)
...
fig.add_trace(go.Scatter(x=epoch_train, y=avg_accuracy_train[0],
                    mode='lines',
                    name='Train Without kmeans'))

fig.add_trace(go.Scatter(x=epoch_train, y=avg_accuracy_test[0],
                    mode='lines',
                    name='Test without kmeans'))
fig.add_trace(go.Scatter(x=epoch_train, y=avg_f1_score_ada[0],
                    mode='lines',
                    name='F1 Score Without kmeans'))

fig.add_trace(go.Scatter(x=epoch_train, y=avg_accuracy_train_gmm[0],
                    mode='lines',
                    name='Train With kmeans'))

fig.add_trace(go.Scatter(x=epoch_train, y=avg_accuracy_test_gmm[0],
                    mode='lines',
                    name='Test With kmeans'))
fig.add_trace(go.Scatter(x=epoch_train, y=avg_f1_score_ada_gmm[0],
                    mode='lines',
                    name='F1 Score With kmeans'))
...
total_times= [round(avg_run_time_gmm,3), round(avg_run_time_without,3)]
experiments = ['With Kmeans', 'Without Kmeans']
colors = ['blue', 'green']
